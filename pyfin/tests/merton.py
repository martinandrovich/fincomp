...
def mainCalculation():
	...
	print(f"{[cf(i) for i in [0, 1, 10, 20, 500]] = }")
	...
	valOur = [merton(option_type="CALL", s0=S0, r=r, sigma=sigma, mu_J=muJ, sigma_J=sigmaJ, xi_p=xiP, K=k, T=tau) for k in K]
	valCOSlOur = [merton_cos(option_type="CALL", s0=S0, r=r, sigma=sigma, mu_J=muJ, sigma_J=sigmaJ, xi_p=xiP, K=float(k), T=tau, a=a, b=b) for k in K]

	print(valCOSlOur)
	plt.figure(1)
	# plt.plot(K,valueExact, label="Exact")
	plt.plot(K,valCOS, label="COS")
	# plt.plot(K,valOur, label="Exact (Our)")
	plt.plot(K,valCOSlOur, label="COS (Our)")
	plt.xlabel("strike, K")
